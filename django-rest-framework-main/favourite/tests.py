...
class FavouriteCreateList(APITestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(title="SedatTitle", content="SedatContent")
...
class FavouriteUpdateDelete(APITestCase):
    ...
    def setUp(self):
        self.username = "sedattest"
        self.password = "sedattest"
        self.user = User.objects.create_user(username=self.username, password=self.password)
        self.user2 = User.objects.create_user(username="sedattest2", password=self.password)
        self.post = Post.objects.create(title="SedatTitle", content="SedatContent")
        self.favourite = Favourite.objects.create(content="xoxoxo", post=self.post, user=self.user)
        self.url = reverse("favourite:update-delete", kwargs={"pk": self.favourite.pk})
        self.test_jwt_authentication()

    def test_jwt_authentication(self, username="sedattest", password="sedattest"):
